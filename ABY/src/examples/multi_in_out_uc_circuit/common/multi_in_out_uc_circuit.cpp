...
#include "multi_in_out_uc_circuit.h"
  ...
  if (file.is_open()) {
    while (getline(file, line)) {
      if (line != "") {
        ...
        if (line.at(0) == 'U') {
          getline(file_xy, line_xy);
          // This also needs different parsing
          //tokenize(line_xy, tokens_xy);
	  std::string inputs = line.substr(0,line.find("|"));
	  std::string outputs = line.substr(line.find("|"));
	  std::vector<uint32_t> input_tokens;
	  std::vector<uint32_t> output_tokens;
	  std::vector<std::vector<uint32_t>> programming_tokens;
          assert(tokens_xy.size() == 1 << (tokens.size() - 2));

          size_t lut_index = 0;
          size_t seperator_index = 1;
          
          std::cout << "Seperator at " << seperator_index << "\n";
          for (size_t j = 1; j < seperator_index; j++) {
            lut_index |= wires_carry[tokens[j]] << (seperator_index - 1 - j);
          }
...
int32_t test_universal_circuit(e_role role, char *address, uint16_t port,
  ...
  /**
       ...
       */
  results =
      circ->PutMultiInOutUniversalCircuitFromFile(filename, p1filename, p2inputs, nvals,sharing);
  ...
  */
  ...
  auto total_data = party->GetSentData(sharing,P_TOTAL) + party->GetReceivedData(sharing,P_TOTAL);
  std::ofstream file;
  std::cout << "Total time / comm: " << party->GetTiming(P_TOTAL) << " ms / "
    ...
    file_out << filename <<"\t" << role << "\t" << sharing  << "\t" << party->GetTiming(P_TOTAL) << "\t" << total_data <<"\t"<< "1"<< std::endl;
    file_out.close();
