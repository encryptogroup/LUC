...
int32_t read_test_options(int32_t *argcp, char ***argvp, e_role *role,
                          ...
                          std::string *filename, std::string *p1filename,
                          uint32_t *nruns, std::string *p2input) {
...
int main(int argc, char **argv) {
  ...
  uint16_t port = 7766;
  std::string address = "130.83.125.167";
  ...
  read_test_options(&argc, &argv, &role, &bitlen, &secparam, &address, &port,
                    &filename, &p1filename, &nruns, &p2input);

  seclvl seclvl = get_sec_lvl(secparam);

  test_universal_circuit(role, (char *)address.c_str(), port, seclvl, nvals,
                         bitlen, nthreads, mt_alg, sharing, filename,
                         p1filename, p2input);
