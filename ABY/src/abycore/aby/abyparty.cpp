...
uint64_t ABYParty::GetSentData(e_sharing sharing, ABYPHASE phase) {
	if(sharing == S_BOOL || sharing == S_SPLUT)
	{
		uint64_t total_sent = 0;
		for(int i = 0; i < m_vSockets.size();i++)
		{
		auto socket = m_vSockets[i].get();
		total_sent += socket->getSndCnt();
		}
		return total_sent;
	}
	return GetSentDataForPhase(phase);
...
uint64_t ABYParty::GetReceivedData(e_sharing sharing, ABYPHASE phase) {
	if(sharing == S_BOOL || sharing == S_SPLUT)
	{
