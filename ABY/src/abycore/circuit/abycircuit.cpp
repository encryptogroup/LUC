...
uint32_t ABYCircuit::PutUniversalGate(uint32_t inleft, uint32_t inright, std::vector<uint32_t> lut_programming, uint32_t nrounds) {
	GATE* gate = InitGate(G_UNIV, inleft, inright);

	gate->nvals = std::min(m_vGates[inleft].nvals, m_vGates[inright].nvals);

        // copy lut programming
        uint32_t* lut_programming_copy = new uint32_t[lut_programming.size()];
        for (size_t i = 0; i < lut_programming.size(); i++)
          lut_programming_copy[i] = lut_programming[i];

	gate->nrounds = nrounds;
	gate->gs.ttable.ptr = lut_programming_copy;
	gate->gs.ttable.len = lut_programming.size();
