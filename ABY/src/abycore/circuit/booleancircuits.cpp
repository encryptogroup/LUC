/**
 ...
 \brief		A collection of boolean circuits for boolean and yao sharing in
 the ABY framework
 */
...
void BooleanCircuit::Init() {
  ...
  if (m_eContext == S_BOOL) {
    ...
    m_nRoundsOUT.resize(3, 1);
    m_nRoundsOUT[1] =
        0; // the client already holds the output bits from the start
  } else {
    std::cerr << "Sharing type not implemented for Boolean circuit"
              << std::endl;
...
uint32_t BooleanCircuit::PutANDGate(uint32_t inleft, uint32_t inright) {
  ...
  if (m_eContext != S_SPLUT) {
    gateid =
        m_cCircuit->PutPrimitiveGate(G_NON_LIN, inleft, inright, m_nRoundsAND);
...
std::vector<uint32_t>
BooleanCircuit::PutANDGate(std::vector<uint32_t> inleft,
                           std::vector<uint32_t> inright) {
...
uint32_t BooleanCircuit::PutVectorANDGate(uint32_t choiceinput,
                                          uint32_t vectorinput) {
  if (m_eContext != S_BOOL) {
    std::cerr
        << "Building a vector AND gate is currently only possible for GMW!"
        << std::endl;
    // TODO: prevent error by putting repeater gate on choiceinput and an AND
    // gate between choiceinput and vectorinput
    return 0;
  }

  uint32_t gateid = m_cCircuit->PutNonLinearVectorGate(
      G_NON_LIN_VEC, choiceinput, vectorinput, m_nRoundsAND);
  UpdateInteractiveQueue(gateid);

  // std::cout << "Putting a vector and gate between a gate with " <<
  // m_vGates[choiceinput].nvals << " and " <<
  // m_vGates[vectorinput].nvals
  // << ",
  // res gate has nvals = " << m_vGates[gateid].nvals << std::endl;

  if (m_vGates[gateid].nvals != INT_MAX) {
    // Update vector AND sizes
    // find location of vector AND bitlength
    // int pos = FindBitLenPositionInVec(m_vGates[gateid].nvals, m_vANDs,
    // m_nNumANDSizes);
    int pos = FindBitLenPositionInVec(m_vGates[gateid].gs.avs.bitlen, m_vANDs,
                                      m_nNumANDSizes);
    if (pos == -1) {
      // Create new entry for the bit-length
      m_nNumANDSizes++;
      non_lin_vec_ctx *temp =
          (non_lin_vec_ctx *)malloc(sizeof(non_lin_vec_ctx) * m_nNumANDSizes);
...
uint32_t BooleanCircuit::PutXORGate(uint32_t inleft, uint32_t inright) {
  // std::cout << "inleft = " << inleft << ", inright = " << inright <<
  // std::endl;
  uint32_t gateid =
      m_cCircuit->PutPrimitiveGate(G_LIN, inleft, inright, m_nRoundsXOR);
  UpdateLocalQueue(gateid);
...
std::vector<uint32_t>
BooleanCircuit::PutXORGate(std::vector<uint32_t> inleft,
                           std::vector<uint32_t> inright) {
  PadWithLeadingZeros(inleft, inright);
  uint32_t resultbitlen = inleft.size();
  std::vector<uint32_t> out(resultbitlen);
...
uint32_t BooleanCircuit::PutINGate(e_role src) {
  uint32_t gateid = m_cCircuit->PutINGate(m_eContext, 1, m_nShareBitLen, src,
                                          m_nRoundsIN[src]);
  UpdateInteractiveQueue(gateid);
  switch (src) {
  case SERVER:
...
uint32_t BooleanCircuit::PutSIMDINGate(uint32_t ninvals, e_role src) {
  uint32_t gateid = m_cCircuit->PutINGate(m_eContext, ninvals, m_nShareBitLen,
                                          src, m_nRoundsIN[src]);
  UpdateInteractiveQueue(gateid);
  switch (src) {
  case SERVER:
...
uint32_t BooleanCircuit::PutSharedSIMDINGate(uint32_t ninvals) {
  uint32_t gateid =
      m_cCircuit->PutSharedINGate(m_eContext, ninvals, m_nShareBitLen);
  UpdateLocalQueue(gateid);
  return gateid;
}
...
uint32_t BooleanCircuit::PutINGate(uint64_t val, e_role role) {
  ...
  if (role == m_eMyRole) {
    ...
    gate->gs.ishare.inval = (UGATE_T *)calloc(
        ceil_divide(1 * m_nShareBitLen, sizeof(UGATE_T) * 8), sizeof(UGATE_T));
...
uint32_t BooleanCircuit::PutSIMDINGate(uint32_t nvals, uint64_t val,
                                       e_role role) {
  // return PutINGate(nvals, &val, role);
  ...
  if (role == m_eMyRole) {
    ...
    GATE *gate = &(m_vGates[gateid]);
    gate->gs.ishare.inval = (UGATE_T *)calloc(
        ceil_divide(nvals * m_nShareBitLen, sizeof(UGATE_T) * 8),
        sizeof(UGATE_T));
...
uint32_t BooleanCircuit::PutYaoSharedSIMDINGate(uint32_t nvals,
                                                yao_fields keys) {
  uint32_t gateid = PutSharedSIMDINGate(nvals);
  // assign value
  GATE *gate = &(m_vGates[gateid]);
...
share *BooleanCircuit::PutYaoSharedSIMDINGate(uint32_t nvals, yao_fields *keys,
                                              uint32_t bitlen) {
...
std::vector<uint32_t>
BooleanCircuit::PutOUTGate(std::vector<uint32_t> parentids, e_role dst) {
  std::vector<uint32_t> gateid =
      m_cCircuit->PutOUTGate(parentids, dst, m_nRoundsOUT[dst]);
...
std::vector<uint32_t>
BooleanCircuit::PutSharedOUTGate(std::vector<uint32_t> parentids) {
...
share *BooleanCircuit::PutSIMDCONSGate(uint32_t nvals, UGATE_T val,
                                       uint32_t bitlen) {
  share *shr = new boolshare(bitlen, this);
  for (uint32_t i = 0; i < bitlen; ++i) {
...
share *BooleanCircuit::PutSIMDCONSGate(uint32_t nvals, uint8_t *val,
                                       uint32_t bitlen) {
  share *shr = new boolshare(bitlen, this);
  for (uint32_t i = 0; i < bitlen; ++i) {
    uint32_t shift = i % 8;
    shr->set_wire_id(
        i, PutConstantGate((val[(i / 8)] & (1 << shift)) >> shift, nvals));
  }
  return shr;
}

share *BooleanCircuit::PutSIMDCONSGate(uint32_t nvals, uint32_t *val,
                                       uint32_t bitlen) {
  share *shr = new boolshare(bitlen, this);
  for (uint32_t i = 0; i < bitlen; ++i) {
    uint32_t shift = i % 32;
    shr->set_wire_id(
        i, PutConstantGate((val[(i / 32)] & (1 << shift)) >> shift, nvals));
  }
  return shr;
}
...
uint32_t BooleanCircuit::PutConstantGate(UGATE_T val, uint32_t nvals) {
  uint32_t gateid =
      m_cCircuit->PutConstantGate(m_eContext, val, nvals, m_nShareBitLen);
  UpdateLocalQueue(gateid);
  return gateid;
}
...
std::vector<uint32_t>
BooleanCircuit::PutINVGate(std::vector<uint32_t> parentid) {
...
uint32_t BooleanCircuit::PutY2BCONVGate(uint32_t parentid) {
  ...
  UpdateLocalQueue(gateid);
  // a Y input gate cannot be parent to a Y2B gate. Alternatively, put a Boolean
  // input gate
...
std::vector<uint32_t>
BooleanCircuit::PutYSwitchRolesGate(std::vector<uint32_t> parentid) {
  std::vector<uint32_t> out(parentid.size());
...
std::vector<uint32_t>
BooleanCircuit::PutY2BCONVGate(std::vector<uint32_t> parentid) {
  std::vector<uint32_t> out(parentid.size());
  for (uint32_t i = 0; i < parentid.size(); i++) {
...
std::vector<uint32_t>
BooleanCircuit::PutB2YCONVGate(std::vector<uint32_t> parentid) {
  std::vector<uint32_t> out(parentid.size());
  for (uint32_t i = 0; i < parentid.size(); i++) {
...
std::vector<uint32_t>
BooleanCircuit::PutA2YCONVGate(std::vector<uint32_t> parentid) {
  ...
  for (uint32_t i = 0; i < m_vGates[parentid[0]].sharebitlen; i++) {
    ...
    m_vGates[srvshares[i]]
        .depth++; // increase depth by 1 since yao is evaluated before arith
    ...
    m_vGates[clishares[i]]
        .depth++; // increase depth by 1 since yao is evaluated before arith
    UpdateInteractiveQueue(clishares[i]);
  }

  m_nA2YGates +=
      m_vGates[parentid[0]].nvals * m_vGates[parentid[0]].sharebitlen;
...
uint32_t
BooleanCircuit::PutStructurizedCombinerGate(std::vector<uint32_t> input,
                                            uint32_t pos_start,
                                            uint32_t pos_incr, uint32_t nvals) {
  uint32_t gateid = m_cCircuit->PutStructurizedCombinerGate(input, pos_start,
                                                            pos_incr, nvals);
  UpdateLocalQueue(gateid);
  return gateid;
}

share *BooleanCircuit::PutStructurizedCombinerGate(share *input,
                                                   uint32_t pos_start,
                                                   uint32_t pos_incr,
                                                   uint32_t nvals) {
  share *out = new boolshare(1, this);
  nstructcombgates++;
  out->set_wire_id(0, PutStructurizedCombinerGate(input->get_wires(), pos_start,
                                                  pos_incr, nvals));
  return out;
}

uint32_t
BooleanCircuit::PutUniversalGate(std::vector<uint32_t> input_wires,
                                 std::vector<uint32_t> lut_programming) {
  uint32_t gateid;

  if (m_eContext == S_YAO) {
    gateid = PutUniversalGateCircuit(input_wires, lut_programming);
...
std::vector<uint32_t>
BooleanCircuit::PutUniversalGate(std::vector<uint32_t> a,
                                 std::vector<uint32_t> b,
                                 std::vector<uint32_t> lut_programming) {
  uint32_t niters = std::min(a.size(), b.size());
  std::vector<uint32_t> output(niters);
  for (uint32_t i = 0; i < niters; i++) {
    std::vector input_gates{a[i], b[i]};
    output[i] = PutUniversalGate(input_gates, lut_programming);
  }
  return output;
}

share *BooleanCircuit::PutUniversalGate(share *a, share *b,
                                        std::vector<uint32_t> lut_programming) {
  return new boolshare(
      PutUniversalGate(a->get_wires(), b->get_wires(), lut_programming), this);
}

uint32_t BooleanCircuit::PutCallbackGate(std::vector<uint32_t> in,
                                         uint32_t rounds,
                                         void (*callback)(GATE *, void *),
                                         void *infos, uint32_t nvals) {

  uint32_t gateid =
      m_cCircuit->PutCallbackGate(in, rounds, callback, infos, nvals);
...
share *BooleanCircuit::PutCallbackGate(share *in, uint32_t rounds,
                                       void (*callback)(GATE *, void *),
                                       void *infos, uint32_t nvals) {
  return new boolshare(
      PutCallbackGate(in->get_wires(), rounds, callback, infos, nvals), this);
}

/*uint64_t* transposeTT(uint32_t dima, uint32_t dimb, uint64_t* ttable) {
        uint32_t longbits = sizeof(uint64_t) * 8;
        uint64_t* newtt = (uint64_t*) calloc(bits_in_bytes(dima * dimb),
sizeof(uint8_t));
        ...
        for(uint32_t i = 0; i < dima; i++) {
                for(uint32_t j = 0; j < dimb; j++) {
                        ...
                        newtt[idxdst / longbits] |= (((ttable[idxsrc / longbits]
>> (idxsrc % longbits)) & 0x01) << (idxdst % longbits));
                }
        }
...
std::vector<uint32_t> BooleanCircuit::PutTruthTableMultiOutputGate(
    std::vector<uint32_t> in, uint32_t out_bits, uint64_t *ttable) {
...
share *BooleanCircuit::PutTruthTableMultiOutputGate(share *in,
                                                    uint32_t output_bitlen,
                                                    uint64_t *ttable) {
  return new boolshare(
      PutTruthTableMultiOutputGate(in->get_wires(), output_bitlen, ttable),
      this);
}

uint32_t BooleanCircuit::PutTruthTableGate(std::vector<uint32_t> in,
                                           uint32_t out_bits,
                                           uint64_t *ttable) {
...
void BooleanCircuit::UpdateTruthTableSizes(uint32_t len, uint32_t gateid,
                                           uint32_t out_bits) {
    ...
    if (len == m_vTTlens[depth][i][0].tt_len) {
      ...
      for (uint32_t j = 0; j < m_vTTlens[depth][i].size() && !outs_exist; j++) {
        if (m_vTTlens[depth][i][j].out_bits == out_bits) {
          ...
          // In case of OP-LUT, also save the truth table which is needed in the
          // setup phase
          if (m_eContext == S_BOOL) {
            for (uint32_t n = 0; n < nvals; n++) {
              m_vTTlens[depth][i][j].ttable_values.push_back(
                  m_vGates[gateid].gs.tt.table);
            }
          }
        }
      }
  ...
  if (!ins_exist) {
    ...
    // In case of OP-LUT, also save the truth table which is needed in the setup
    // phase
    if (m_eContext == S_BOOL) {
      for (uint32_t n = 0; n < nvals; n++) {
        m_vTTlens[depth][old_in_lens][0].ttable_values.push_back(
            m_vGates[gateid].gs.tt.table);
      }
    }
    outs_exist = true;
  ...
  if (!outs_exist) {
    ...
    // m_vTTlens[depth][id][old_out_lens].tt_len = len;//should work without
    // this too
    ...
    if (m_eContext == S_BOOL) {
      for (uint32_t n = 0; n < nvals; n++) {
        m_vTTlens[depth][id][old_out_lens].ttable_values.push_back(
            m_vGates[gateid].gs.tt.table);
      }
    }
    outs_exist = true;
...
share *BooleanCircuit::PutLeftShifterGate(share *in, uint32_t pos) {
  return new boolshare(PutLeftShifterGate(in->get_wires(),
                                          in->get_max_bitlength(), pos,
                                          in->get_nvals()),
                       this);
}
...
std::vector<uint32_t>
BooleanCircuit::PutLeftShifterGate(std::vector<uint32_t> val,
                                   uint32_t max_bitlen, uint32_t pos,
                                   uint32_t nvals) {
...
uint32_t
BooleanCircuit::PutUniversalGateCircuit(std::vector<uint32_t> input_wires,
                                        std::vector<uint32_t> lut_programming) {
  uint32_t nvals = 0;
  for (uint32_t input_wire : input_wires) {
    nvals = std::max(nvals, m_vGates[input_wire].nvals);
  }

  size_t num_gates = lut_programming.size();
  for (size_t i = 0; i < input_wires.size(); i++) {
    num_gates += (1 << i);
  }

  std::vector<uint32_t> gates(num_gates);
  for (size_t i = 0; i < lut_programming.size(); i++) {
    assert(lut_programming[i] == 0 || lut_programming[i] == 1);
    gates[i] = PutConstantGate(lut_programming[i], nvals);
  }

  size_t gate_counter = 0;
  size_t mux_counter = lut_programming.size();
  for (size_t i = input_wires.size(); i > 0; i--) {
    size_t num_mux = (1 << (i - 1));
    for (size_t mux_i = mux_counter; mux_i < mux_counter + num_mux; mux_i++) {
      uint32_t a = gates[gate_counter++];
      uint32_t b = gates[gate_counter++];
      gates[mux_i] = PutVecANDMUXGate(b, a, input_wires[i - 1]);
    }
    mux_counter += num_mux;
  }
...
share *BooleanCircuit::PutADDGate(share *ina, share *inb) {
  // also output the carry of the result as long as the additional carry does
  // not exceed the maximum bit length of the higher of both inputs
  bool carry = std::max(ina->get_bitlength(), inb->get_bitlength()) <
               std::max(ina->get_max_bitlength(), inb->get_max_bitlength());
  return new boolshare(PutAddGate(ina->get_wires(), inb->get_wires(), carry),
                       this);
}

std::vector<uint32_t> BooleanCircuit::PutAddGate(std::vector<uint32_t> left,
...
std::vector<uint32_t>
BooleanCircuit::PutSizeOptimizedAddGate(std::vector<uint32_t> a,
                                        std::vector<uint32_t> b, BOOL bCarry) {
  ...
  C[0] =
      PutXORGate(a[0], a[0]); // PutConstantGate(0, m_vGates[a[0]].nvals); //the
                              // second parameter stands for the number of vals
...
// TODO: there is a bug when adding 3 and 1 as two 2-bit numbers and expecting a
// carry
std::vector<uint32_t>
BooleanCircuit::PutDepthOptimizedAddGate(std::vector<uint32_t> a,
                                         std::vector<uint32_t> b, BOOL bCARRY,
                                         bool vector_and) {
  PadWithLeadingZeros(a, b);
  uint32_t id, inputbitlen = std::min(a.size(), b.size());
  std::vector<uint32_t> out(a.size() + bCARRY);
  std::vector<uint32_t> parity(a.size()), carry(inputbitlen),
      parity_zero(inputbitlen);
  ...
  for (uint32_t i = 1; i <= (uint32_t)ceil(log(inputbitlen) / log(2)); i++) {
    for (uint32_t j = 0; j < inputbitlen; j++) {
      if (j % (uint32_t)pow(2, i) >= pow(2, (i - 1))) {
        id = pow(2, (i - 1)) + pow(2, i) * ((uint32_t)floor(j / (pow(2, i)))) -
             1;
        if (m_eContext == S_BOOL && vector_and) {
          ...
          // carry[j] = PutINVGate(PutANDGate(PutINVGate(s_out->get_wire(0)),
          // PutINVGate(carry[j])));
          carry[j] = PutXORGate(s_out->get_wire_id(0), carry[j]);
          parity[j] = s_out->get_wire_id(1);
        } else {
          // carry[j] = PutINVGate(PutANDGate(PutINVGate(PutANDGate(parity[j],
          // carry[id])), PutINVGate(carry[j]))); // c = (p and c-1) or c = (((p
          // and c-1) xor 1) and (c xor 1)) xor 1)
          carry[j] = PutXORGate(
              carry[j],
              PutANDGate(parity[j],
                         carry[id])); // c = c XOR (p and c-1), from ShallowCC
...
// A carry-save adder
std::vector<std::vector<uint32_t>> BooleanCircuit::PutCarrySaveGate(
    std::vector<uint32_t> a, std::vector<uint32_t> b, std::vector<uint32_t> c,
    uint32_t inbitlen, bool carry) {
...
/*
 * In implementation of the Brent-Kung adder for the Bool-No-MT sharing. To
 * process the values, 5 LUTs are needed: 1) for the inputs, 2) for intermediate
 * carry-forwarding, 3) for critical path on inputs, 4) for the critical path,
 * 5) for the inverse carry tree.
 */
std::vector<uint32_t> BooleanCircuit::PutLUTAddGate(std::vector<uint32_t> a,
                                                    std::vector<uint32_t> b,
                                                    BOOL bCARRY) {
  uint32_t inputbitlen = std::max(a.size(), b.size());
  PadWithLeadingZeros(a, b);
  std::vector<uint32_t> out(a.size() + bCARRY);
  std::vector<uint32_t> parity(inputbitlen), carry(inputbitlen),
      parity_zero(inputbitlen), tmp;
  ...
  // std::cout << "Building a LUT add gate for " << inputbitlen << " input bits"
  // << std::endl;
  ...
  // std::cout << "building a crit path input gate with nins = " << n_crit_ins
  // << std::endl;
  tmp = PutTruthTableMultiOutputGate(
      lut_in, n_crit_ins, (uint64_t *)m_vLUT_ADD_CRIT_IN[n_crit_ins - 1]);
  ...
  for (uint32_t i = n_crit_ins; i < inputbitlen;) {
    ...
    // std::cout << "building a standard input gate with nins = " <<
    // processed_ins << ", i = " << i << " and first val = " << (hex) <<
    // m_vLUT_ADD_IN[processed_ins-1][0] <<(dec)<<
    //		", lut_in.size() = " << lut_in.size() << ", expecting nouts = "
    //<< m_vLUT_ADD_N_OUTS[processed_ins-1] << std::endl; std::cout << "Inputs:
    // ";
    ...
    // process inputs via LUT and write updated gates into carry / parity
    // vectors
    tmp = PutTruthTableMultiOutputGate(
        lut_in, m_vLUT_ADD_N_OUTS[processed_ins - 1],
        (uint64_t *)m_vLUT_ADD_IN[processed_ins - 1]);
    // std::cout << ", outputs = " << i;
    carry[i] = tmp[0];
    if (processed_ins > 1) {
      // std::cout << ", " << i+1;
  ...
  for (uint32_t d = 1; d < ceil_log2(inputbitlen + 1) / 2; d++) {
    ...
    // std::cout << "critical intermediate base = " << base << ", dist = " <<
    // dist << ", processed_ins = " << processed_ins << std::endl;
    ...
    // std::cout << "building a crit-path lut with " << lut_in.size() << " input
    // wires: " << base-1-dist << ", ";
    /*for(uint32_t j = 0; j < processed_ins; j++) {
    ...
    std::cout << std::endl;*/
    tmp = PutTruthTableMultiOutputGate(
        lut_in, processed_ins, (uint64_t *)m_vLUT_ADD_CRIT[processed_ins - 1]);
    for (uint32_t j = 0; j < tmp.size(); j++) {
      carry[base - 1 + j * dist] = tmp[j];
    }

    // step 2.2: forward carry and parity signals down the tree
    for (uint32_t i = (base + 3 * dist) - 1; i + dist < inputbitlen;
         i += (4 * dist)) {
      processed_ins = std::min(
          ceil_divide((inputbitlen - (i + dist)), 2 * dist), max_ins - 2);
      // std::cout << "intermediate base = " << i << ", dist = " << dist << ",
      // processed_ins = " << processed_ins << std::endl;

      lut_in.clear();
      lut_in.resize(4 * processed_ins);
      // std::cout << "building an internal lut with " << lut_in.size() << "
      // input wires: ";
      ...
      // std::cout << std::endl;
      tmp = PutTruthTableMultiOutputGate(
          lut_in, processed_ins * 2,
          (uint64_t *)m_vLUT_ADD_INTERNAL[processed_ins - 1]);
      // std::cout << "Truth table = " <<
      // m_vLUT_ADD_INTERNAL[processed_ins-1][0] << std::endl;
      for (uint32_t j = 0; j < tmp.size() / 2; j++) {
        // std::cout << "writing bit " << 2*j << " and " << 2*j+1 << " to
        // parity/carry position " << i+dist+2*j*dist << std::endl;
  ...
  // std::cout << "Doing " << (floor_log2(inputbitlen/5)/2)+1 << " iterations on
  // the inverse carry tree, " << floor_log2(inputbitlen/5) << ", " <<
  // inputbitlen/5 << std::endl; step 3: build the inverse carry tree d
  // increases with d = 0: 5, d = 1: 20, d = 2: 80; d = 3: 320, ...
  for (int32_t d = (floor_log2(inputbitlen / 5) / 2); d >= 0; d--) {
    ...
    // std::cout << "d = " << d << ", start = " << start << ", dist = " << dist
    // << std::endl;
    for (uint32_t i = start; i < inputbitlen; i += start) {
      ...
      if (processed_ins > 0) {
        ...
        // std::cout << "Building INV gate with " << processed_ins << " inputs:
        // " << i-1;
        /*for(uint32_t j = 0; j < processed_ins; j++) {
        ...
        // std::cout << std::endl;
        tmp = PutTruthTableMultiOutputGate(
            lut_in, processed_ins,
            (uint64_t *)m_vLUT_ADD_INV[processed_ins - 1]);
  ...
  // step 4: compute the outputs from the carry signals and the parity bits at
  // the zero-th level
  out[0] = parity_zero[0];
  for (uint32_t i = 1; i < inputbitlen; i++) {
    out[i] = PutXORGate(parity_zero[i], carry[i - 1]);
...
std::vector<uint32_t> BooleanCircuit::PutMulGate(std::vector<uint32_t> a,
                                                 std::vector<uint32_t> b,
                                                 uint32_t resultbitlen,
  ...
  if (m_eContext == S_BOOL && vector_ands) {
    ...
    for (uint32_t i = 0; i < inputbitlen; i++) {
      PutMultiMUXGate(&ina, &zero_share, inb->get_wire_ids_as_share(i), 1,
                      &(mulout[i]));
    }
...
#ifdef ZDEBUG
    std::cout << "Starting to construct multiplication gate for " << inputbitlen
              << " bits" << std::endl;
#endif
    ...
    for (uint32_t i = 0, ctr; i < inputbitlen; i++) {
      ...
      vAdds[i].resize(resultbitlen);
#ifdef ZDEBUG
      std::cout << "New Iteration with ctr = " << ctr
                << ", and resultbitlen = " << resultbitlen << std::endl;
#endif
      for (uint32_t j = 0; j < i && ctr < resultbitlen; j++, ctr++) {
        vAdds[i][ctr] = zerogate;
      }
...
share *BooleanCircuit::PutMULGate(share *ina, share *inb) {
  // set the resulting bit length to be the smallest of: 1) bit length of the
  // products or 2) the highest maximum bit length between ina and inb
  uint32_t resultbitlen =
      std::min(ina->get_bitlength() + inb->get_bitlength(),
               std::max(ina->get_max_bitlength(), inb->get_max_bitlength()));
  return new boolshare(
      PutMulGate(ina->get_wires(), inb->get_wires(), resultbitlen), this);
}
...
share *BooleanCircuit::PutMUXGate(share *ina, share *inb, share *sel) {
  return new boolshare(
      PutMUXGate(ina->get_wires(), inb->get_wires(), sel->get_wire_id(0)),
      this);
}

std::vector<uint32_t>
BooleanCircuit::PutWideAddGate(std::vector<std::vector<uint32_t>> ins) {
  ...
  while (survivors.size() > 1) {
    ...
    for (unsigned i = 0; i < survivors.size();) {
      if (i + 1 >= survivors.size()) {
        ...
        survivors[j++] =
            PutSizeOptimizedAddGate(survivors[i], survivors[i + 1], false);
        i += 2;
      }
    }
    survivors.resize(j);
...
BooleanCircuit::PutCSNNetwork(std::vector<std::vector<uint32_t>> ins) {
  ...
  while (wires > 2) {
    for (; c_tail < c_head - 2; c_tail += 3) {
      ...
      out = PutCarrySaveGate(carry_lines[c_tail], carry_lines[c_tail + 1],
                             carry_lines[c_tail + 2], inputbitlen);
#ifdef ZDEBUG
      std::cout << "Computing Carry CSA for gates " << survivors[p_tail] << ", "
                << survivors[p_tail + 1] << ", " << survivors[p_tail + 2]
                << " and bitlen: " << (2 * inputbitlen - 1)
                << ", gates before: " << temp_gates
                << ", gates after: " << m_nFrontier << std::endl;
#endif
...
#endif
      // temp_gates = m_nFrontier;
      out = PutCarrySaveGate(survivors[p_tail], survivors[p_tail + 1],
                             survivors[p_tail + 2], inputbitlen);
#ifdef ZDEBUG
      std::cout << "Computing Parity CSA for gates " << survivors[p_tail]
                << ", " << survivors[p_tail + 1] << ", "
                << survivors[p_tail + 2]
                << " and bitlen: " << (2 * inputbitlen - 1)
                << ", gates before: " << temp_gates
    ...
    if ((p_head - p_tail) < 3 && (c_head - c_tail) < 3 && wires > 2) {
      ...
      rem[1] =
          (p_head - p_tail) > 1 ? survivors[p_tail + 1] : carry_lines[c_tail];
      rem[2] =
          (p_head - p_tail) > 1 ? carry_lines[c_tail] : carry_lines[c_tail + 1];
      rem[3] = left > 3 ? carry_lines[c_tail + 1]
                        : dummy; // the dummy value should never be used!
...
#ifdef ZDEBUG
        std::cout << "Computing Finish CSA for gates " << rem[j] << ", "
                  << rem[j + 1] << ", " << rem[j + 2]
                  << " and bitlen: " << (2 * inputbitlen - 1)
                  << ", wires: " << wires << ", gates before: " << temp_gates
                  << ", gates after: " << m_nFrontier << std::endl;
#endif
        rem[left++] = out[0];
        rem[left++] = out[1];
...
#ifdef ZDEBUG
      std::cout << "Computed last CSA gate, wires = " << wires << ", ending "
                << std::endl;
#endif
    }
  }
...
std::vector<uint32_t> BooleanCircuit::PutSUBGate(std::vector<uint32_t> a,
                                                 std::vector<uint32_t> b,
                                                 uint32_t max_bitlength) {
...
share *BooleanCircuit::PutSUBGate(share *ina, share *inb) {
  return new boolshare(
      PutSUBGate(ina->get_wires(), inb->get_wires(),
                 std::max(ina->get_max_bitlength(), inb->get_max_bitlength())),
      this);
}

// computes: ci = a > b ? 1 : 0; but assumes both values to be of equal length!
uint32_t BooleanCircuit::PutGTGate(std::vector<uint32_t> a,
                                   std::vector<uint32_t> b) {
  PadWithLeadingZeros(a, b);
...
// computes: ci = a > b ? 1 : 0; but assumes both values to be of equal length!
uint32_t BooleanCircuit::PutSizeOptimizedGTGate(std::vector<uint32_t> a,
                                                std::vector<uint32_t> b) {
  PadWithLeadingZeros(a, b);
...
uint32_t BooleanCircuit::PutDepthOptimizedGTGate(std::vector<uint32_t> a,
                                                 std::vector<uint32_t> b) {
  PadWithLeadingZeros(a, b);
  uint32_t i, rem;
  ...
  for (i = 0; i < inputbitlen; i++) {
    agtb[i] = PutANDGate(
        a[i], PutINVGate(b[i])); // PutBitGreaterThanGate(a[i], b[i]);
  }
  ...
  while (rem > 1) {
    uint32_t j = 0;
    // std::cout << "New iter with " << size << " element remaining"<<
    // std::endl;
    for (i = 0; i < rem;) {
      if (i + 1 >= rem) {
        agtb[j] = agtb[i];
...
#ifdef ZDEBUG
  std::cout << "Finished greater than tree with adress: " << agtb[0]
            << ", and bitlength: " << a.size() << std::endl;
#endif
  return agtb[0];
}

uint32_t BooleanCircuit::PutLUTGTGate(std::vector<uint32_t> a,
                                      std::vector<uint32_t> b) {
  ...
  for (uint32_t i = 0; i < state.size();) {
    ...
    tmp = PutTruthTableMultiOutputGate(
        lut_ins, 2, (uint64_t *)m_vLUT_GT_IN[(nins / 2) - 1]);
    // std::cout << "using lut " << (hex) << m_vLUT_GT_IN[(nins/2)-1][0] <<
    // (dec) << std::endl;
  ...
  while (state.size() > 2) {
    ...
    for (uint32_t i = 0; i < state.size();) {
      ...
      // it is not efficient to build a gate here so copy the wires to the next
      // level
      if (nins <= minins && state.size() > minins) {
        ...
        lut_ins.assign(state.begin() + i, state.begin() + i + nins);
        tmp = PutTruthTableMultiOutputGate(
            lut_ins, 2, (uint64_t *)m_vLUT_GT_INTERNAL[nins / 2 - 2]);
        state[j] = tmp[0];
        state[j + 1] = tmp[1];
...
uint32_t BooleanCircuit::PutEQGate(std::vector<uint32_t> a,
                                   std::vector<uint32_t> b) {
  PadWithLeadingZeros(a, b);
...
std::vector<uint32_t> BooleanCircuit::PutORGate(std::vector<uint32_t> a,
                                                std::vector<uint32_t> b) {
...
share *BooleanCircuit::PutANDVecGate(share *ina, share *inb) {
  ...
  if (m_eContext == S_BOOL) {
    for (uint32_t i = 0; i < inputbitlen; i++) {
      out->set_wire_id(
          i, PutVectorANDGate(inb->get_wire_id(i), ina->get_wire_id(i)));
    }
  } else {
    // std::cout << "Putting usual AND gate" << std::endl;
    for (uint32_t i = 0; i < inputbitlen; i++) {
      uint32_t bvec = PutRepeaterGate(inb->get_wire_id(i),
                                      m_vGates[ina->get_wire_id(i)].nvals);
...
/* if s == 0 ? b : a*/
std::vector<uint32_t> BooleanCircuit::PutMUXGate(std::vector<uint32_t> a,
                                                 std::vector<uint32_t> b,
                                                 uint32_t s, BOOL vecand) {
...
share *BooleanCircuit::PutVecANDMUXGate(share *a, share *b, share *s) {
  return new boolshare(
      PutVecANDMUXGate(a->get_wires(), b->get_wires(), s->get_wires()), this);
}

/* if s == 0 ? b : a*/
std::vector<uint32_t> BooleanCircuit::PutVecANDMUXGate(
    std::vector<uint32_t> a, std::vector<uint32_t> b, std::vector<uint32_t> s) {
...
uint32_t BooleanCircuit::PutWideGate(e_gatetype type,
  ...
  while (survivors.size() > 1) {
    ...
    for (uint32_t i = 0; i < survivors.size();) {
      ...
      if (nins <= minins && survivors.size() > minins) {
        ...
        lut[((1L << nins) - 1) / table_bitlen] =
            1L << (((1L << nins) - 1) % table_bitlen);
...
share **BooleanCircuit::PutCondSwapGate(share *a, share *b, share *s,
                                        BOOL vectorized) {
  share **s_out = (share **)malloc(sizeof(share *) * 2);

  std::vector<std::vector<uint32_t>> out = PutCondSwapGate(
      a->get_wires(), b->get_wires(), s->get_wire_id(0), vectorized);
...
std::vector<std::vector<uint32_t>>
BooleanCircuit::PutCondSwapGate(std::vector<uint32_t> a,
                                std::vector<uint32_t> b, uint32_t s,
                                BOOL vectorized) {
  std::vector<std::vector<uint32_t>> out(2);
  uint32_t inputbitlen = std::max(a.size(), b.size());
  PadWithLeadingZeros(a, b);
...
std::vector<uint32_t> BooleanCircuit::PutELM0Gate(std::vector<uint32_t> val,
                                                  uint32_t b) {
...
std::vector<uint32_t>
BooleanCircuit::PutMaxGate(const std::vector<std::vector<uint32_t>> &ws) {
  BinaryOp_v_uint32_t op = [this](auto a, auto b) {
    uint32_t cmp = (m_eContext == S_YAO) ? this->PutSizeOptimizedGTGate(a, b)
...
std::vector<uint32_t>
BooleanCircuit::PutMinGate(std::vector<std::vector<uint32_t>> a) {
  // build a balanced binary tree
  uint32_t cmp;
...
void BooleanCircuit::PutMinIdxGate(share **vals, share **ids, uint32_t nvals,
                                   share **minval_shr, share **minid_shr) {
...
// vals = values, ids = indicies of each value, n = size of vals and ids
void BooleanCircuit::PutMinIdxGate(std::vector<std::vector<uint32_t>> vals,
                                   std::vector<std::vector<uint32_t>> ids,
                                   std::vector<uint32_t> &minval,
                                   std::vector<uint32_t> &minid) {
  // build a balanced binary tree
  uint32_t cmp;
...
// vals = values, ids = indicies of each value, n = size of vals and ids
void BooleanCircuit::PutMaxIdxGate(share **vals, share **ids, uint32_t nvals,
                                   share **maxval_shr, share **maxid_shr) {
  std::vector<std::vector<uint32_t>> val(nvals);
  std::vector<std::vector<uint32_t>> id(nvals);
...
// vals = values, ids = indicies of each value, n = size of vals and ids
void BooleanCircuit::PutMaxIdxGate(std::vector<std::vector<uint32_t>> vals,
                                   std::vector<std::vector<uint32_t>> ids,
...
std::vector<uint32_t> BooleanCircuit::PutFPGate(const std::string func,
...
std::vector<uint32_t> BooleanCircuit::PutFPGate(const std::string func,
                                                std::vector<uint32_t> ina,
                                                std::vector<uint32_t> inb,
                                                uint8_t bitsize,
                                                uint32_t nvals) {
...
std::vector<uint32_t>
BooleanCircuit::PutGateFromFile(const std::string filename,
                                std::vector<uint32_t> inputs, uint32_t nvals) {
  std::string line;
  ...
  if (myfile.is_open()) {
    while (getline(myfile, line)) {
      ...
      if (line != "") {
        ...
        case 'M': // MUX Gate
          wires[tokens[3]] = PutVecANDMUXGate(
              wires[tokens[1]], wires[tokens[0]], wires[tokens[2]]);
          break;
    ...
    if (file_input_size < inputs.size()) {
      std::cerr << "Warning: Input sizes didn't match! Less inputs read from "
                   "circuit file than passed to it!"
                << std::endl;
    }
  }
...
std::vector<uint32_t> BooleanCircuit::PutUniversalCircuitFromFile(
    const std::string filename, const std::string p1filename,
    std::vector<uint32_t> p2inputs, uint32_t nvals) {
  std::string line;
  std::string p1line;
  ...
  std::vector<std::vector<uint32_t>> p1inputsgate;
  ...
  if (!p1file.is_open()) {
    std::cerr << "Error: Unable to open programming file " << p1filename
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
#ifdef DEBUG_UC
  ...
  while (getline(p1file, p1line)) {
    if (p1line != "") {
      ...
      if (m_eMyRole == SERVER) {
        p1inputsgate.push_back(p1tokens);
      } else {
        p1inputsgate.push_back(std::vector<uint32_t>(p1tokens.size(), 0));
      }
#ifdef DEBUG_UC
  ...
  while (getline(myfile, line)) {
    ...
    if (line != "") {
...
#endif
        wires[tokens[2]] = PutVecANDMUXGate(wires[tokens[0]], wires[tokens[1]],
                                            p1inputs[p1_counter]);
        p1_counter++;
        break;
      ...
      case 'A': // AND Gate
#ifdef DEBUG_UC
        std::cout << "AND Gate " << tokens[2] << " = A(" << tokens[0] << " , "
                  << tokens[1] << ") " << std::endl;
#endif
        wires[tokens[2]] = PutANDGate(wires[tokens[0]], wires[tokens[1]]);
        break;
      ...
      case 'E': // XOR Gate
#ifdef DEBUG_UC
        std::cout << "XOR Gate " << tokens[2] << " = E(" << tokens[0] << " , "
                  << tokens[1] << ") " << std::endl;
#endif
        wires[tokens[2]] = PutXORGate(wires[tokens[0]], wires[tokens[1]]);
        break;
      ...
      case 'I': // INV Gate
#ifdef DEBUG_UC
        std::cout << "INV Gate " << tokens[1] << " = I(" << tokens[0] << ") "
                  << std::endl;
#endif
        wires[tokens[1]] = PutINVGate(wires[tokens[0]]);
        break;
      ...
      case 'U': // Universal Gate
        ...
        std::cout << "Universal Gate " << tokens[2] << " = U(" << tokens[0]
                  << " , " << tokens[1] << ") op = " << p1inputs[p1_counter]
                  << std::endl;
#endif
        input_wires.clear();
        for (size_t i = 0; i < tokens.size() - 1; i++) {
          input_wires.push_back(wires[tokens[i]]);
        }

        wires[tokens[tokens.size() - 1]] =
            PutUniversalGate(input_wires, p1inputsgate[p1_counter]);
        p1_counter++;
        break;
      ...
      case 'X': // X Gate
        ...
        std::cout << "X Gate (" << tokens[2] << " , " << tokens[3] << ")"
                  << "= X(" << tokens[0] << " , " << tokens[1]
                  << ") c = " << p1inputs[p1_counter] << std::endl;
#endif
        tmp_wire1.clear();
        tmp_wire2.clear();
  ...
  if (file_input_size < p2inputs.size()) {
    std::cerr << "Warning: Input sizes didn't match! Less inputs read from "
                 "circuit file than passed to it! "
              << file_input_size << "  " << p2inputs.size() << std::endl;
  }

  return outputs;
}

std::vector<uint32_t> BooleanCircuit::PutMultiInOutUniversalCircuitFromFile(
    const std::string filename, const std::string p1filename,
    std::vector<uint32_t> p2inputs, uint32_t nvals, e_sharing sharing) {
  std::string line;
  std::string p1line;
  std::vector<uint32_t> tokens, outputs, p1tokens;
  std::map<uint32_t, uint32_t> wires;
  ...
  std::vector<std::vector<uint32_t>> p1inputsgate;
  std::vector<uint32_t> input_wires{};
  std::vector<uint32_t> inputline_tokens;
  std::vector<uint32_t> outputline_tokens;
  std::vector<uint32_t> multilut_out_wires;
  size_t prog_size;
  std::vector<uint32_t> programming_output;
  std::string inputstring;
  std::string outputstring;
  std::vector<uint32_t> programming;
  std::ifstream p1file;

  uint32_t ttable_vals;
  ...
  while (getline(p1file, p1line)) {
    if (p1line != "") {
      p1line.erase(remove(p1line.begin(), p1line.end(), '|'), p1line.end());
      tokenize(p1line, p1tokens);
#ifdef DEBUG_UC
      ...
      if (m_eMyRole == SERVER) {
        ...
        if (GetContext() == S_SPLUT || GetContext() == S_BOOL) {
          p1inputsgate.push_back(p1tokens);
        } else {
          p1inputsgate.push_back(std::vector<uint32_t>(p1tokens.size(), 0));
        }
  ...
  while (getline(myfile, line)) {
    ...
    if (line != "") {
      ...
      case 'U': // Universal Gate
        ...
        input_wires.clear();
        inputline_tokens.clear();
        outputline_tokens.clear();
        inputstring = line.substr(1, line.find("|") - 1);
        // everything after |
        outputstring = line.substr(line.find("|") + 1);
        // HERE PARSE MULTI INPUT MULTI OUTPUT
        // tokenize_verilog(line, tokens);
        inputline_tokens.clear();
        outputline_tokens.clear();
        tokenize(inputstring, inputline_tokens);
        tokenize(outputstring, outputline_tokens);

        for (size_t i = 0; i < inputline_tokens.size(); i++) {
          input_wires.push_back(wires[inputline_tokens[i]]);
        }

        prog_size = pow(2, input_wires.size());

        ttable = (uint32_t *)calloc(1,ceil_divide((1 << input_wires.size()) * outputline_tokens.size(), 8 * 8) * 8);
	// ttable = (uint32_t *) malloc(8);
        programming = p1inputsgate[p1_counter];
        if (!(GetContext() == S_SPLUT || GetContext() == S_BOOL)) {
          for (size_t i = 0; i < outputline_tokens.size(); i++) {
            programming_output.clear();
            // tokenize substring of programming information
            for (size_t j = 0; j < prog_size; j++) {
              programming_output.push_back(programming[i * prog_size + j]);
            }

            wires[outputline_tokens[i]] =
                PutUniversalGate(input_wires, programming_output);
          }
          // wires[outputline_tokens[i]] = PutUniversalGate(input_wires,
          // p1inputsgate[p1_counter]);
        } else {
          ...
          for (int i = 0; i < prog_size; i++) {
            for (int j = 0; j < outputline_tokens.size(); j++) {
              if (c == 64) {
                c = 0;
                k++;
              }
              ...
              ttable[k] =
                  ttable[k] | (programming[j * prog_size + i]
                               << (((i)*outputline_tokens.size() + j) % 64));

              c++;
            }
          }
          std::reverse(input_wires.begin(), input_wires.end());
          multilut_out_wires = PutTruthTableMultiOutputGate(
              input_wires, outputline_tokens.size(), (uint64_t *)ttable);
          for (uint32_t i = 0; i < multilut_out_wires.size(); i++) {
            wires[outputline_tokens[i]] = multilut_out_wires[i];
          }
        }
...
void BooleanCircuit::GetInputLengthFromFile(const std::string filename,
                                            uint32_t &client_input,
                                            uint32_t &server_input) {
  std::string line;
...
share *BooleanCircuit::PutLUTGateFromFile(const std::string filename,
                                          share *input) {
  return new boolshare(PutLUTGateFromFile(filename, input->get_wires()), this);
}

std::vector<uint32_t>
BooleanCircuit::PutLUTGateFromFile(const std::string filename,
                                   std::vector<uint32_t> inputs) {
  std::string line;
  std::vector<uint32_t> tokens, outputs;
  std::map<uint32_t, uint32_t> wires;
    ...
    while (getline(myfile, line)) {
      ...
      if (line != "") {
        ...
        case 'L': // Parse LUT
          ...
          // std::cout << "n_inputs: " << n_inputs << ", n_outputs = " <<
          // n_outputs << std::endl; next will follow n_inputs input wires.
          // Prepare to assign them to a temporary vector for later use
          ...
          // next up are the contents of the LUT for each output wire, which is
          // broken into ceil(2^{n_inputs} / 32) 32-bit values. For each output
          // wire, we first have the content, followed by the id of the output
          // wire.
          token_outputs.resize(n_outputs);
          ttable_vals = ceil_divide(1 << n_inputs, sizeof(uint32_t) * 8);
          ttable = (uint32_t *)malloc(
              ceil_divide(1 << n_inputs, sizeof(UGATE_T) * 8) * n_outputs *
              sizeof(UGATE_T));
          // std::cout << "(" << ttable_vals << ") TTable values : " <<
          // std::endl;;
          ...
          // Build the LUT gate
          lut_outputs = PutTruthTableMultiOutputGate(lut_inputs, n_outputs,
                                                     (uint64_t *)ttable);
        ...
        case 'O': // map the output wires from the gate to the output of this
                  // function
...
void BooleanCircuit::PutMultiMUXGate(share **Sa, share **Sb, share *sel,
                                     uint32_t nshares, share **Sout) {
  ...
  for (uint32_t i = 0, idx; i < bitlen; i++) {
    for (uint32_t j = 0, ctr = 0; j < nshares && (i >= ctr || j == 0); j++) {
      if (i < (ctr + Sa[j]->get_bitlength())) {
        idx = i - ctr;
        // std::cout << "for i = " << i << " taking j = " << j << " and ctr = "
        // << ctr << std::endl;
  ...
  for (uint32_t i = 0, idx; i < bitlen; i++) {
    for (uint32_t j = 0, ctr = 0; j < nshares && (i >= ctr || j == 0); j++) {
      if (i < (ctr + Sa[j]->get_bitlength())) {
        ...
        Sout[j]->set_wire_id(
            idx,
            PutStructurizedCombinerGate(out, i, bitlen, nvals)->get_wire_id(0));
      }
      ctr += Sa[j]->get_bitlength();
    }
  }
...
void BooleanCircuit::PadWithLeadingZeros(std::vector<uint32_t> &a,
                                         std::vector<uint32_t> &b) {
...
share *BooleanCircuit::PutFullAdderGate(uint32_t a, uint32_t b,
                                        uint32_t carry_in) {
  std::vector<uint32_t> out(2);

#ifdef FA_DEBUG
  std::vector<uint32_t> v_a(1);
  v_a[0] = a;
  std::vector<uint32_t> v_b(1);
  v_b[0] = b;
  std::vector<uint32_t> v_c_in(1);
  v_c_in[0] = carry_in;
...
share *BooleanCircuit::PutADDChainGate(std::vector<uint32_t> a,
                                       std::vector<uint32_t> b,
                                       uint32_t carry_in) {
  PadWithLeadingZeros(a, b);
...
#ifdef AC_DEBUG
    PutPrintValueGate(
        new boolshare(std::vector<uint32_t>(&a[i], &a[i + 1]), this), "a i");
    PutPrintValueGate(
        new boolshare(std::vector<uint32_t>(&b[i], &b[i + 1]), this), "b i");
...
share *BooleanCircuit::PutHammingWeightGateRec(uint32_t *wires,
                                               uint32_t bitlen) {
  share *out;
  ...
  if (bitlen > 3) {
    ...
    std::cout << "Input bitlen: " << bitlen << "\tBitlen v: " << bitlen_v
              << "\tBitlen u: " << bitlen_u
              << "\tBitlen i: " << bitlen - bitlen_v - bitlen_u << std::endl;
#endif
    // build v
    ...
    delete v;
    if (bitlen_u > 0)
      delete u; // u == zero_share otherwise and deleted later
  } else if (bitlen > 2)
...
share *BooleanCircuit::PutConvTypeGate(share *value, ConvType *from,
                                       ConvType *to, uint32_t nvals) {
  return new boolshare(PutConvTypeGate(value->get_wires(), from, to, nvals),
                       this);
}

std::vector<uint32_t>
BooleanCircuit::PutConvTypeGate(std::vector<uint32_t> wires, ConvType *from,
                                ConvType *to, uint32_t nvals) {
  switch (to->getType()) {
  case ENUM_FP_TYPE:
...
std::vector<uint32_t>
BooleanCircuit::PutUint2FpGate(std::vector<uint32_t> wires,
                               [[maybe_unused]] UINTType *from, FPType *to,
                               uint32_t nvals) {
  ...
  reversed_preor.insert(reversed_preor.begin(), prefix_or.rbegin(),
                        prefix_or.rend());
  ...
  value = new boolshare(
      PutBarrelLeftShifterGate(s_in->get_wires(), value->get_wires(), nvals),
      this);
...
#ifdef UINT2FP_DEBUG
  std::cout << "HW out size: " << pre_or_for_exp->get_wires().size()
            << std::endl;
  PutPrintValueGate(pre_or_for_exp, "pre or for exp");
#endif

  share *exp = PutSIMDCONSGate(nvals, (uint64_t)(to->getExpBias() - 1),
                               to->getExpBits());
...
std::vector<uint32_t> BooleanCircuit::PutFp2UintGate(std::vector<uint32_t>
wires, FPType* from, UINTType* to){ std::vector<uint32_t> out; std::cout <<
"PutFP2INTGate is not implemented yet" << std::endl; std::exit(EXIT_FAILURE);
        return out;
}
*/
...
std::vector<uint32_t>
BooleanCircuit::PutPreOrGate(std::vector<uint32_t> wires) {
...
share *BooleanCircuit::PutBarrelLeftShifterGate(share *input, share *n) {
  return new boolshare(
      PutBarrelLeftShifterGate(input->get_wires(), n->get_wires()), this);
}

std::vector<uint32_t> BooleanCircuit::PutBarrelLeftShifterGate(
    std::vector<uint32_t> wires, std::vector<uint32_t> n, uint32_t nvals) {
...
share *BooleanCircuit::PutBarrelRightShifterGate(share *input, share *n) {
  return new boolshare(
      PutBarrelRightShifterGate(input->get_wires(), n->get_wires()), this);
}

std::vector<uint32_t>
BooleanCircuit::PutBarrelRightShifterGate(std::vector<uint32_t> wires,
...
share *BooleanCircuit::PutFPGate(share *in, op_t op, uint8_t bitlen,
                                 uint32_t nvals, fp_op_setting s) {
...
share *BooleanCircuit::PutFPGate(share *in_a, share *in_b, op_t op,
                                 uint8_t bitlen, uint32_t nvals,
                                 fp_op_setting s) {
  // if bitlen/nvals were not set manually, use values from input
  if (bitlen == 0) {
  ...
  return new boolshare(
      PutFPGate(o, in_a->get_wires(), in_b->get_wires(), bitlen, nvals), this);
}
